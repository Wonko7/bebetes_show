...
void init_cases (ccase ** c,int * nb_case)
{
  ...
  *c = malloc(NB_CASES * (sizeof (ccase)));
  ...
  (*c)[SABLE].name = "sable";
  (*c)[SABLE].texture = Load_image("sable.bmp");
  (*c)[SABLE].model = NULL;
  (*c)[SABLE].objet = NULL;
  (*c)[SABLE].accessible = 1;

  (*c)[HERBE].name = "herbe";
  (*c)[HERBE].texture = Load_image("herbe.bmp");
  (*c)[HERBE].model = NULL;
  (*c)[HERBE].objet = NULL;
  (*c)[HERBE].accessible = 1;

  (*c)[EAU].name = "eau";
  (*c)[EAU].texture = Load_image("eau.bmp");
  (*c)[EAU].model = NULL;
  (*c)[EAU].objet = NULL;
  (*c)[EAU].accessible = 0;

  (*c)[PALMIER].name = "palmier";
  (*c)[PALMIER].texture = Load_image("herbe.bmp");
  (*c)[PALMIER].model = "palmier";
  (*c)[PALMIER].objet = load_objet("palmier",0.25);;
  (*c)[PALMIER].accessible = 0;
...
{
  int i,j,k,t[]={5,9,13,16,19,21,23,24,25};
  x_et_y *tabu,piou;
...
void init_stimuli (behavior ** s,int *nb_sti, float *cycle)
{
  ...
  *s = malloc(NB_STIMULI * (sizeof (behavior)));
  ...
  (*s)[NOTHING_TO_DO].ag = C;
  (*s)[MARCHER].name_s = "Marcher";
  (*s)[MARCHER].ag = C;
...
void init_iaction (iaction **ia,int *nb_ia)
{
  ...
  *ia = malloc(NB_IA * (sizeof (iaction)));
  ...
  (*ia)[PICKING].name_ia = "Suivre un chameau";
  (*ia)[PICKING].ia = suivre;
  (*ia)[PICKING].i = 0;
  
  (*ia)[TUER].name_ia = "Tuer un chameau";
  (*ia)[TUER].ia = ia_tuer;
  (*ia)[TUER].i = 1;
}
